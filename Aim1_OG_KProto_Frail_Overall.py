...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 28 14:07:12 2025
...
df_train = pd.read_csv('/file_path/filename.csv')
df_testing = pd.read_csv('/file_path/filename.csv')

#Join the dataframes
df = pd.concat([df_train, df_testing], ignore_index=True, axis=0)
...
df_array_kproto.to_csv('/filepath/filename.csv', index=False)
#Label the original dataframe
df_original_combined['Cluster_Labels'] = original_labels
df_original_combined.to_csv('/filepath/filename.csv', index=False)
...
df_final_scores.to_csv('/filepath/filename.csv', index=False)
